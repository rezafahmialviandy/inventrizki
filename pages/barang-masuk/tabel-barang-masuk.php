...
<div class="table-responsive">
    <table class="table table-bordered table-striped table-hover" id="tabel_barang_masuk" width="100%" cellspacing="0">
        <thead>
        <tr>
            <th>No</th>
            <th>Tanggal</th>
        ...
        <tbody>
            <?php
                ...
                if (isset($_POST['bulan'])){
                    $bulan=$_POST['bulan'];
                    $tahun=$_POST['tahun'];
                    $sql="select d.*,b.nama_barang,t.tanggal from detail_barang_masuk d inner join barang b on b.kode_barang=d.kode_barang inner join barang_masuk t on t.kode_transaksi=d.kode_transaksi  where month(t.tanggal)='".$bulan."' and year(t.tanggal)='".$tahun."' order by t.tanggal desc";
                }else {
                    $sql="select d.*,b.nama_barang,t.tanggal from detail_barang_masuk d inner join barang b on b.kode_barang=d.kode_barang inner join barang_masuk t on t.kode_transaksi=d.kode_transaksi order by t.tanggal desc";
                }
            ...
            <tr>
                ...
                <?php if ($_SESSION["level"]=="admin"): ?>
                <td>
                    <button class="tombol_edit_barang btn btn-warning btn-circle btn-sm" kode_barang="<?php echo $data['kode_barang']; ?>" kode_transaksi="<?php echo $data['kode_transaksi'];?>" kode_barang="<?php echo $data['kode_barang'];?>" data-toggle="tooltip" title="Edit" data-placement="top" ><i class="fa fa-edit"></i></button>
...
$('.tombol_edit_barang').on('click',function(){
    ...
    $.ajax({
        url: 'pages/barang-masuk/edit-barang.php',
        method: 'post',
        data: {kode_transaksi:kode_transaksi,kode_barang:kode_barang},
        success:function(data){
            $('#tampil_data').html(data);  
                document.getElementById("judul").innerHTML='Edit Barang Masuk';
        }
    });
    // Membuka modal
    $('#modal').modal('show');
...
$('.tombol_hapus_barang').on('click',function(){
    ...
    if (!agree){
        ...
        $.ajax({
            url: 'pages/barang-masuk/ajax/hapus-barang.php',
            method: 'post',
            data: {kode_transaksi:kode_transaksi,kode_barang:kode_barang},
            success:function(data){
                tabel_barang();
